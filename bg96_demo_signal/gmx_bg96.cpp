...
uint8_t gmxBG96_getSignal(char *data, uint8_t max_size)
{
...
uint8_t gmxBG96_signalInfo(int *rssi, int *rsp, int *sinr, int *rsrq, char *dump){
  ...
  memset(data,0,sizeof(data));
  if ( at_copy_serial_to_buffer(data, '\r', 128,  BG96_DEFAULT_TIMEOUT) != AT_COMMAND_SUCCESS )
    return GMX_BG96_KO;
