...
void setup() {
   ...
   // if (  gmxBG96_setGSM(confAPN) == 0 ) 
    ...
    if ( gmxBG96_setNBIoT(confAPN,"80000") == 0) 
    {
...
void loop() {
  ...
  /*
   * First condition: if the timer reach 0 and if the device is attached,
   * we send temperature data (and display signal info in Serial)
   * 
   * NB: if you do not need temperature and just want signal values,
   * you could delete part 2.
   */
  if ((( delta_tx > timer_period_to_tx) || (timer_millis_tx == 0 )) && network_attached )
  {
    if ( gmxBG96_isNetworkAttachedStatus() )
    {
      ...
      //Clear the dispaly of the screen
      SeeedOled.clearDisplay();
  ...
  /*
   * Second condition: if we are not sending temperature info (ie, timer not to 0)
   * we gather and display the signal information on OLed Screen
   * 
   */
  else
  {
    //gmxBG96_signalInfo take signal's value (see gmx_bg96.cpp library)
    if ( gmxBG96_signalInfo( &rssi, &rsrp, &sinr, &rsrq, dump ) == 0 )
    {
      sinr = sinr/5 - 20;
      //display the value on the screen (see display_utils.cpp library)
      displaySignal(rssi,1,"RSSI");
      displaySignal(rsrp,2,"RSRP");
      displaySignal(sinr,3,"SINR");
      displaySignal(rsrq,4,"RSRQ");
    } 
    else
    {
      _debug("SIGN_KO");
  ...
  /*
   * Checking that device still attached.
   * If not, re-attached
   */
  ...
  /*
   * NB: Could be deleted if part 2 has been deleted (temperature not needed then).
   */
